...
def device(request):
    return render(request, 'device.html', {'computers': Device.objects.all()})


def users(request):
    return render(request, 'users.html', {'users': Owner.objects.all()})
        ...
        if adresses:
            ...
            try:
                for device in device_info:
                    devices = Device(
                        ip_address=device['IP'],  mac_address=device['MAC Address'],  hostname=device['Hostname'], os=device['Os'])
                    devices.save()
...
def check_port(request):
    host, port = request.POST.get(
        'hostname', False), request.POST.get('port', False)
...
def check_localisation(request):
    ...
    if lat == None or long == None:
        ...
        context = {'m': m, }
        messages.error(
            request, 'localisation does not exist or check your network connection')
